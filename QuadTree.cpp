...
{
     for_loop(Index, Particles::ParticleCount)
     {
         Particles::ParticleList[Index]->Render();
     }
}


void Particles::ClampPosition()
{
...
bool  Node::Insert(Particles *object)
{
    if(IsContained(object))               // If the Object Fits in this Square
    {
        if(IsLeaf == true)                // And its a Leaf Node
        {
            ...
            else                       // Else If its full than make the Node a Branch
            {
                ...
                for(Particles *object: Entities)  // Put the Particles that were in this Node in their respective SubNodes
                {
                   PushDown(object);
                }
...
bool  Node::IsContained(Particles *object)
{
...
bool  Node::PushDown(Particles *object)
{
    if(SubNodes[NE]->Insert(object) == true) return true;
    if(SubNodes[SE]->Insert(object) == true) return true;
    if(SubNodes[NW]->Insert(object) == true) return true;
...
void  Node::Moveup(Particles *object)
{
...
void QT::Init()
{
    ...
    for_loop(Index, Particles::ParticleCount)
    {
        RootNode->Insert(Particles::ParticleList[Index]);
    }
}
